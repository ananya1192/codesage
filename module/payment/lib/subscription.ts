...
export async function getRemainingLimits(userId: string): Promise<UserLimits> {
    ...
    for (const repo of repositories) {
        ...
        limits.reviews[repo.id] = {
            ...
            canAdd:
  tier === "PRO" ||
  repositoryCount < TIER_LIMITS.FREE.repositories,
        };
    }
...
const DAILY_AI_REVIEW_LIMIT = 20;
...
export async function checkAIRateLimit(userId: string): Promise<void> {
    ...
    let usage = await prisma.aIRateLimit.findUnique({
        where: {
            userId_date: {
                userId,
                date: today,
    ...
    if (!usage) {
        usage = await prisma.aIRateLimit.create({
            data: {
                userId,
                date: today,
                reviewCount: 0,
    ...
    if (usage.reviewCount >= DAILY_AI_REVIEW_LIMIT) {
        throw new Error(
            "Daily AI review limit reached. Upgrade to Pro for unlimited reviews."
        );
    }
}

export async function incrementAIRateLimit(
  userId: string
): Promise<void> {
  const today = new Date().toISOString().split("T")[0];
